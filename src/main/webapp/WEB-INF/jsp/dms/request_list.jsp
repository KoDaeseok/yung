    ...
    <div class="page-container">
        ...
        <main class="page-content">
            <div class="breadcrumb">
                ...
                <span>></span><span>요청/리서치자료</span><span>></span><span>${menuDetail.menuNm}</span>
            </div>
                        ...
                        <c:choose>
                            <c:when test="${not empty list}">
                                <c:forEach var="item" items="${list}">
                                    <tr data-href="/dms/request/detail?id=${item.id}">
                                        <td>${item.regNum}</td>
                                        <td>${item.team}</td>
                                        ...
                                        <td>${item.endDate}</td>
                                        <td>
                                            <span class="status-badge ${item.status == '등록완료' ? 'complete' : 'pending'}">${item.status}</span>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </c:when>
